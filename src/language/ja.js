  ...
  modal: {
    toast: {
      ...
      warning: {
        ...
        saltMissing: "サーバーがデクリプションソルトの変更またはデリートを検出しました。これにより、既存のエンクリプトされたファイルがリカバーできなくなる可能性があります。サーバーが保存したすべてのローカルソルトバックアップは、次のパスにあります：{0}",
        autoSaveFailed: "同期にフェイルしました。別のファイルに切り替えるか、エディタを再起動するまで、オートセーブは一時的に無効になります。",
